...
<layout:override name="<%=Blocks.BLOCK_HEADER_SCRIPTS%>">
    ...
    <script type="text/javascript">
        ...
        function openDoop(btn, did, doorId, name){
            if(window.confirm("确认开启"+name+"吗？")){
                $(btn).prop("disabled", true).text("正在尝试开启"+name+"...");
                $.post("/relay/openDrive", {did: did}, function(res) {
        ...
        function closeDoop(btn, did, doorId, name){
            if(window.confirm("确认关闭"+name+"吗？")){
                $(btn).prop("disabled", true).text("正在尝试关闭"+name+"...");
                $.post("/relay/closeDrive", {did: did}, function(res) {
                    if (res.code == 1 && res.data.status == 1) {
        ...
        function openLock(btn, did, lockId){
            if(window.confirm("确认开启锁吗？")){
                $(btn).prop("disabled", true).text("正在尝试开启锁...");
                $.post("/relay/openLock", {did: did, lockId: lockId}, function(res) {
                    if (res.code == 1 && res.data.status == 1) {
        ...
        function closeLock(btn, did, lockId){
            if(window.confirm("确认关闭锁吗？")){
                $(btn).prop("disabled", true).text("正在尝试关闭锁...");
                $.post("/relay/closeLock", {did: did, lockId: lockId}, function(res) {
                    if (res.code == 1 && res.data.status == 1) {
        ...
        function readEpcLabel(btn, did){
            $(btn).prop("disabled", true).text("正在读取..");
            $("#distribution_code").val("");
            $("#testReads").html("");
            $.post("/relay/readEpcLabelCK", {did: did}, function(res){
                if(res.code == 1 && res.data.rfids){
                    $("#testReads").html('<p>读到以下标签：共'+res.data.rfids.length+'个</p>');
                    $.each(res.data.rfids, function (index, item) {
    ...
    <div class="container-fluid">
        <div class="animated fadeIn">
            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="card">
                                        <div class="card-body p-3 d-flex align-items-center">
                                            <c:if test="${status=='1'}">
                                                <font color="green">硬件已连接（连接时间：${connDate}）</font>
                                            </c:if>
                                            <c:if test="${status=='2'}">
                                                <font color="red">硬件已断开</font>
                                            </c:if>
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <div class="card-body p-3 d-flex align-items-center">
                                                    ...
                                                    <div class="mr-5">
                                                        ...
                                                        <div class="text-muted text-uppercase font-weight-bold small">
                                                            <span data-door-open-${d.lockId} class="badge badge-success" <c:if test="${d.readStatus == '0'}">style="display: none;" </c:if>>开${d.name}中</span>
                                                            <span data-door-close-${d.lockId} class="badge badge-danger" <c:if test="${d.readStatus == '1'}">style="display: none;"</c:if>>关${d.name}中</span>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <button data-door-close-${d.lockId} class="btn btn-success" <c:if test="${d.readStatus == '1'}">style="display: none;"</c:if> onclick="openDoop(this, '${d.did}', '${d.lockId}', '${d.name}')">开${d.name}</button>
                                                        <button data-door-open-${d.lockId} class="btn btn-danger" <c:if test="${d.readStatus == '0'}">style="display: none;"</c:if> onclick="closeDoop(this, '${d.did}', '${d.lockId}', '${d.name}')">关${d.name}</button>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="card">
                                            <div class="card-body p-3 d-flex align-items-center">
                                                ...
                                                <div class="mr-5">
                                                    <div class="text-value-sm text-primary">${d.name}</div>
                                                    <div class="text-muted text-uppercase font-weight-bold small">
                                                        <span data-lock-open-${d.lockId} class="badge badge-success" <c:if test="${d.readStatus == '0'}">style="display: none;" </c:if>>开锁中</span>
                                                        <span data-lock-close-${d.lockId} class="badge badge-danger" <c:if test="${d.readStatus == '1'}">style="display: none;"</c:if>>关锁中</span>
                                                    </div>
                                                </div>
                                                <div>
                                                    <button data-lock-close-${d.lockId} class="btn btn-success" <c:if test="${d.readStatus == '1'}">style="display: none;"</c:if> onclick="openLock(this, '${d.did}', '${d.lockId}')">开 锁</button>
                                                    <button data-lock-open-${d.lockId} class="btn btn-danger" <c:if test="${d.readStatus == '0'}">style="display: none;"</c:if> onclick="closeLock(this, '${d.did}', '${d.lockId}')">关 锁</button>
                                                </div>
                                            </div>
                                            <%--<div class="card-footer px-3 py-2">
                                                <a class="btn-block text-muted d-flex justify-content-between align-items-center" href="#">
                ...
                <!--射频-->
                <div class="col-md-6"  <c:if test="${status=='2'}">style="display: none;" </c:if>>
                    <div class="card">
                        <div class="card-header">
                            <strong>${deviceInfo.name}</strong>
                            <div class="card-block row">
